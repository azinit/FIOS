...
def result(item, flag, mode, sub='', decore=lambda x: x, init=None,
    ...
    col_sign = lambda name, color: _f.paint(_obj.get(name), _f.enhance(color))
    col_obj = lambda val, colors, sign: _f.paint(str(sign + ' '*(len(sign) > 0)) + str(decore(_init(val).content)), colors[flag], c_end)
    def_sign = lambda val, ind: _init(val).sign if init[ind] is True else init[ind]
    ...
    s_true = '' if not sub else (_f.enhance(i_true) if not s_true else s_true)
    ...
    c_end = '' if general_c else _f.end
    sub, init[1] = (sub + ' ', init[1]) if sub or sub == 0 else ('', '')

    flag, sign = _init(flag).property, [col_sign(n, c) for n, c in zip(["arrow", "arrow_r", "arrow_broken", "flag", "flag"], [i_true, i_true, i_false, i_true, i_none])]
...
def parameters(*args, marker=_obj.get("element"), color=_f.beige, general_color=''):
    color = general_color if general_color else color
    if isinstance(args[0], dict):
        for key in dict(args[0]).keys():
            print(general_color + marker, key + ":", _f.paint(args[0].get(key), color))

    else:
        for x in args:
            print(marker, _f.paint(x, color))
...
if __name__ == "__main__":
    waiting()
    for i, i_mode in enumerate(["create", "rename", "clean", "sort", "move", "open", "navigator", "read", "write", "cur_dir", "index"]):
    ...
    for flagg in [False, True, None]:
        process(mode="navigator", flag=flagg, color=_f.beigebg, pat='.')
    for i in range(2, 5):
        print(i)
